...
const tabs = [
  { value: "dashboard", label: "Painel Principal", icon: LayoutDashboard, component: DashboardPanel },
    ...
    <div className="min-h-screen flex flex-col">
      <header className="sticky top-0 z-50 border-b border-cyan-500/15 bg-black/80 backdrop-blur-xl">
        <div className="mx-auto max-w-6xl px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold font-display tracking-wider text-gradient">NeuroLeitor</h1>
              <p className="text-[10px] text-zinc-500 -mt-0.5 tracking-widest uppercase">BioData Reader v0.5</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className={`hidden sm:flex items-center gap-1.5 text-xs ${online ? "text-green-400" : "text-yellow-400"}`}>
              <span className={`h-1.5 w-1.5 rounded-full ${online ? "bg-green-400 animate-pulse" : "bg-yellow-400"}`} />
              {apiLoading ? "API…" : online ? `API online · OCR ${providers?.ocr || "local"}` : "API offline (mock)"}
            </div>
            <a href="https://github.com/producerdcs-cpu/neuro-leitor" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-zinc-400 hover:text-cyan-300">
        ...
        <Tabs value={active} onValueChange={setActive}>
          ...
          {tabs.map((tab) => (
            <TabsContent key={tab.value} value={tab.value}>
              <motion.div key={tab.value} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}>
                <tab.component />
              </motion.div>
            </TabsContent>
          ))}
      ...
      <footer className="border-t border-cyan-500/10 py-4">
        <div className="mx-auto max-w-6xl px-4 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-zinc-600">
          <p>© 2026 NeuroLeitor · Producer DCS · <span className="text-cyan-600">v0.5.0</span></p>
          <p className="font-mono">React · Vite · Tailwind · Fase 3</p>
        </div>
      </footer>
    </div>
  );
